 ...
 * forever.
 */
export function CashierSessionGate({
  children,
  canBypass = false,
}: {
  ...
  // «اسمك» — الحساب مشترك، والاسم يُطبع على الوصل. آخر اسم على هذا الجهاز يُذكر
  const [who, setWho] = useState("");
  useEffect(() => {
    try {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time read of the device's last name
      setWho(localStorage.getItem("st-cashier-name") ?? "");
    } catch {
      /* private mode */
    }
  }, []);
  ...
  async function start(fromSession: string | null, amount: number) {
    ...
    try {
      const res = await openSession({
        float: amount,
        fromSession,
        counted: fromSession ? amount : null,
        cashierName: who,
      });
      try {
        if (who.trim()) localStorage.setItem("st-cashier-name", who.trim());
      } catch {
        /* private mode */
      }
      if (!res.ok) {
  ...
  if (bypass && !session) {
    return (
      <div className="space-y-4">
        <p className="rounded-xl border-2 border-border bg-secondary px-3 py-2 text-sm font-black">
          الكاشير مفتوح بلا وردية — بيع الإدارة يُحسب في اليوم لا في تقرير
          الوردية.
        </p>
        {children}
      </div>
  ...
  const bypassBtn = canBypass && (
    <button
      ...
      disabled={busy}
      className="min-h-12 w-full rounded-2xl border-2 border-primary font-black text-primary"
    >
    ...
    return (
      ...
      >
        <p className="text-sm font-bold text-muted-foreground">
          عُدّ النقد في الصندوق الآن وأدخل ما وجدته فعلاً. إن اختلف عن المبلغ
          أعلاه، سجّله كما هو — الفرق هو الفائدة كلها من هذه الخطوة.
        </p>
        <input
          value={who}
          onChange={(e) => setWho(e.target.value)}
          placeholder="👤 اسمك — يُطبع على الوصل"
          maxLength={60}
          className="min-h-12 w-full rounded-xl border border-input bg-background px-3 text-base outline-none focus:ring-2 focus:ring-ring"
        />
        ...
        <button
          onClick={() => void start(handover.session_id, 0)}
          disabled={busy}
          className="min-h-12 w-full rounded-2xl border-2 border-border font-black"
        >
        ...
        {counted > 0 && counted !== handover.amount && (
          <p className="rounded-xl border-2 border-destructive bg-destructive/10 px-3 py-2 text-sm font-black text-destructive">
            فرق {formatIqd(Math.abs(counted - handover.amount))} د.ع{" "}
            {counted < handover.amount ? "نقصاً" : "زيادة"} — سيُسجَّل للمدير.
          </p>
        )}
        {err && <Err msg={err} />}
      </Card>
  ...
  if (!session) {
    return (
      <Card
        ...
        tone="plain"
        title="بدء الوردية"
        subtitle="أدخل المبلغ الافتتاحي في الصندوق"
      >
        <input
          value={who}
          onChange={(e) => setWho(e.target.value)}
...
function SessionBar({
  ...
  async function openReport() {
    ...
    try {
      ...
      if (!r)
        setErr(
          "تعذّر جلب تقرير الوردية — يمكنك المتابعة، لكن راجع الأرقام يدوياً.",
        );
    } catch (e) {
  ...
  async function finish(countedOverride?: number) {
    ...
    try {
      const res = await closeSession({
        ...
        counted: countedOverride ?? counted,
        deposited,
        note,
      });
  ...
  const since = new Intl.DateTimeFormat("ar-IQ", {
    ...
    <>
      <div className="touch-pos flex flex-wrap items-center justify-between gap-3 rounded-2xl border-2 border-primary bg-primary/5 p-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs font-bold text-muted-foreground">
              الوردية مفتوحة منذ {since}
            </p>
            <p className="font-black">
              {session.cashier_name} · افتتاحي{" "}
              {formatIqdLabel(session.opening_float)}
            </p>
          </div>
        </div>
        <button
              ...
              <>
                ...
                {report ? (
                  <dl className="space-y-1.5 rounded-2xl border-2 border-border p-3 text-sm font-bold">
                    ...
                    <Line
                      label="مبيعات كي كارد (خارج الصندوق)"
                    ...
                    <p className="pt-1 text-xs font-bold text-muted-foreground">
                      {report.orders_count} طلب في هذه الوردية
                    </p>
                  </dl>
                ) : (
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <button
                    ...
                    disabled={busy}
                    className="min-h-14 rounded-xl border-2 border-border font-black"
                  >
                    رجوع
                  </button>
                  <button
                ...
                <button
                  ...
                  disabled={busy}
                  className="mt-2 min-h-12 w-full rounded-xl border-2 border-border font-black"
                >
...
function Result({
  ...
  done: { expected: number; variance: number };
  onClose: () => void;
}) {
    ...
    <div className="space-y-4 text-center">
      ...
      >
        ...
        <p className="text-3xl font-black tabular-nums">
          {ok
            ? "مطابق ✓"
            : `${short ? "نقص" : "زيادة"} ${formatIqd(Math.abs(done.variance))}`}
        </p>
      </div>
      <p className="text-xs font-bold text-muted-foreground">
        سُجّل التقرير. الكاشير التالي سيُطالَب بعدّ الصندوق وتأكيد استلامه قبل
        أن يبيع.
      </p>
      <button
        onClick={onClose}
        className="min-h-14 w-full rounded-xl bg-primary font-black text-primary-foreground"
      >
        تمام
      </button>
    </div>
...
function Line({
  ...
  return (
    ...
    >
      <dt>{label}</dt>
      <dd
        className={`tabular-nums ${tone === "plus" ? "text-primary" : tone === "minus" ? "text-destructive" : ""}`}
      >
...
function Err({ msg }: { msg: string }) {
  return (
    <p className="rounded-xl border-2 border-destructive bg-destructive/10 px-3 py-2 text-sm font-bold text-destructive">
      {msg}
    </p>
  );
}
    ...
    <div className="touch-pos mx-auto max-w-md space-y-4 pt-8">
      ...
      >
        <div className="flex items-center gap-3">
          <span
            className={`flex size-14 items-center justify-center rounded-2xl ${tone === "warn" ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"}`}
          >
            {icon}
          </span>
          <div>
