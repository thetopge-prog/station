...
const SPELLINGS: Record<string, string> = {
  ...
  ميل: "وجبه", وجبات: "وجبه",
  // ما كتبه الزبائن فعلاً ولم يُفهَم — كلٌّ منها من رسالةٍ وصلت. «برقر لحم
  // برقر قجاج و بيزة وسط» ضاع منها البرجر والبيتزا معاً، والهيكل لا يُنقذها:
  // يُسقط الألف والواو والياء لا القاف عن الجيم، فتبقى «برقر» غير «برجر».
  برقر: "برجر", برغر: "برجر", بركر: "برجر", برجرات: "برجر",
  بيزه: "بيتزا", بيزا: "بيتزا", بتزا: "بيتزا", بيتزه: "بيتزا",
  قجاج: "دجاج", دياي: "دجاج", جكن: "دجاج", chicken: "دجاج",
  زنكر: "زنجر", زينجر: "زنجر",
};
...
function splitOrder(text: string): string[] {
  return normDigits(text)
    .replace(/[×x*]\s*(\d+)/gi, " $1 ")
    // الفصل على «و» المنفصلة، **وعلى الملتصقة بعدد**: «واحد بيتزا واثنين
    // برجر» مقطعٌ واحد عند الأولى وحدها، وفي المقطع الواحد يأخذ الصنف الثاني
    // كمّية ١ دائماً — فكان الزبون يطلب اثنين ويصله واحد
    .split(/\n|،|,|\+|\s+و\s+|\s+(?=و(?:احد|حده|اثنين|ثنين|ثلاث|اربع|خمس|ست|سبع|ثمان|تسع|عشر))/)
    .map((s) => s.trim())
    .filter(Boolean);
}
...
function parseSegment(seg: string, menu: Menu): CartLine[] | null {
  ...
  for (const w of words) {
    const n = NUM_WORDS[w] ?? (w.startsWith("و") ? NUM_WORDS[w.slice(1)] : undefined);
    if (n && qty === 1) qty = n;
  }
  const isNum = (w: string) => Boolean(NUM_WORDS[w] ?? (w.startsWith("و") ? NUM_WORDS[w.slice(1)] : undefined));
