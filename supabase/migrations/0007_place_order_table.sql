-- 0007_place_order_table.sql — record the table number (from per-table QR ?t=N)
-- on self-orders. Replaces place_order with a 4-arg version (p_table); the old
-- 3-arg overload is dropped so PostgREST never faces ambiguity. Idempotent.

drop function if exists public.place_order(public.order_channel, jsonb, uuid);
...
begin
  ...
  insert into orders(business_day, order_seq, channel, status, customer_id, cashier_id, table_no)
    values (v_day, v_seq, p_channel, 'pending', p_customer, v_cashier, nullif(trim(coalesce(p_table, '')), ''))
